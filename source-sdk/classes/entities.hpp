...
public:
	...
	int draw_model(int flags, uint8_t alpha) {
		...
		return utilities::get_virtual<original_fn>(animating(), 9)(animating(), flags, alpha);
	}
...
public:
	...
	weapon_info_t* get_weapon_data() {
		auto index = this->item_definition_index();
		if (index == 0x2FAA)
			auto idnex = *reinterpret_cast<short*>(this + 0x2FAA);
			return interfaces::weapon_system->get_weapon_data(this->item_definition_index());

		return interfaces::weapon_system->get_weapon_data(index);
	}
...
public:
	...
	weapon_t* active_weapon() {
		auto active_weapon = read<uintptr_t>(netvar_manager::get_netvar_offset("DT_CSPlayer", "m_hActiveWeapon")) & 0xFFF;
		return reinterpret_cast<weapon_t*>(interfaces::entity_list->get_client_entity(active_weapon));
	}

	UINT* get_wearables() {
		return (UINT*)((uintptr_t)this + (netvar_manager::get_netvar_offset("DT_CSPlayer", "m_hMyWearables")));
	}

	UINT* get_weapons() {
		return (UINT*)((uintptr_t)this + (netvar_manager::get_netvar_offset("DT_CSPlayer", "m_hMyWeapons")));
	}

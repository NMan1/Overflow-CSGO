	...
	std::ofstream("csgo\\materials\\sensum_regular.vmt") << R"#("VertexLitGeneric"
{
  ...
  "$flat"         "1"
	...
	std::ofstream("csgo\\materials\\sensum_flat_xqz.vmt") << R"#("UnlitGeneric"
	{
	  "$basetexture" "vgui/white"
	  "$ignorez"      "1"
	  "$envmap"       ""
	...
	std::ofstream("csgo\\materials\\sensum_regular_xqz.vmt") << R"#("VertexLitGeneric"
{
  "$basetexture" "vgui/white"
  "$ignorez"      "1"
  "$envmap"       ""
...
void features::visuals::dme_chams_run(hooks::dme::fn original, void* thisptr, void* context, void* state, const model_render_info_t& info, matrix3x4_t* custom_bone_to_world)
{
	...
	if (strstr(interfaces::model_info->get_model_name(info.model), "models/player"))
	{
		...
		if (!mat || !mat_xqz || index != menu.config.chams_type)
			...
			{
			case 0:
				mat = interfaces::material_system->find_material("sensum_regular", TEXTURE_GROUP_MODEL); index = menu.config.chams_type;
				mat_xqz = interfaces::material_system->find_material("sensum_regular_xqz", TEXTURE_GROUP_MODEL); index = menu.config.chams_type; break;
			case 1:
				mat = interfaces::material_system->find_material("sensum_reflective", TEXTURE_GROUP_MODEL, true, nullptr); index = menu.config.chams_type;
				mat_xqz = interfaces::material_system->find_material("sensum_reflective_xqz", TEXTURE_GROUP_MODEL); index = menu.config.chams_type; break;
			case 2:
				mat = interfaces::material_system->find_material("sensum_flat", TEXTURE_GROUP_MODEL); index = menu.config.chams_type; 
				mat_xqz = interfaces::material_system->find_material("sensum_flat_xqz", TEXTURE_GROUP_MODEL); index = menu.config.chams_type; break;
			case 3:

...
namespace ImGui
{
	...
	IMGUI_API void          EndChild(bool shadow, ImFont* font = nullptr, ImVec4 color = {});
	...
	IMGUI_API bool          SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format = "%.3f", float power = 1.0f, float width = 275);     // adjust display_format to decorate the value with a prefix or a suffix for in-slider labels or unit display. Use power!=1.0 for logarithmic sliders

...
void ImGui::EndChild(bool shadow, ImFont* font, ImVec4 color) {
	ImGuiWindow* window = GetCurrentWindow();
	...
	if (shadow) {
		...
		ImVec2 text_position(window->Pos.x + 12.0f, window->Pos.y + CalcTextSize(window_title.c_str()).y - 34.0f);

		ImVec2 line_min(text_position.x, window->Pos.y);
		ImVec2 line_max;

		if (window_title.find("Auto") != std::string::npos)
			line_max = ImVec2(text_position.x + CalcTextSize(window_title.c_str()).x + 20, GetWindowPos().y);
		else if (window_title.find("Backtrack") != std::string::npos || window_title.find("Weapons") != std::string::npos)
			line_max = ImVec2(text_position.x + CalcTextSize(window_title.c_str()).x - 14, GetWindowPos().y);
		else
			line_max = ImVec2(text_position.x + CalcTextSize(window_title.c_str()).x + 12, GetWindowPos().y);
		...
		//ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x - 1, text_position.y - 1), color.x > 0 ? ImGui::ColorConvertFloat4ToU32(color) : IM_COL32(0, 0, 0, 255), window_title.c_str());
		//ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x - 1, text_position.y), color.x > 0 ? ImGui::ColorConvertFloat4ToU32(color) : IM_COL32(0, 0, 0, 255), window_title.c_str());
		//ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x - 1, text_position.y + 1), color.x > 0 ? ImGui::ColorConvertFloat4ToU32(color) : IM_COL32(0, 0, 0, 255), window_title.c_str());
		//ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x, text_position.y - 1), color.x > 0 ? ImGui::ColorConvertFloat4ToU32(color) : IM_COL32(0, 0, 0, 255), window_title.c_str());
		//ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x, text_position.y + 1), color.x > 0 ? ImGui::ColorConvertFloat4ToU32(color) : IM_COL32(0, 0, 0, 255), window_title.c_str());
		//ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x + 1, text_position.y - 1), color.x > 0 ? ImGui::ColorConvertFloat4ToU32(color) : IM_COL32(0, 0, 0, 255), window_title.c_str());
		//ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x + 1, text_position.y), color.x > 0 ? ImGui::ColorConvertFloat4ToU32(color) : IM_COL32(0, 0, 0, 255), window_title.c_str());
		//ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x + 1, text_position.y + 1), color.x > 0 ? ImGui::ColorConvertFloat4ToU32(color) : IM_COL32(0, 0, 0, 255), window_title.c_str());
		auto pos = window_title.find("#");
		if (pos != std::string::npos)
			if (pos = window_title.find("#", pos + 1) != std::string::npos)
				if (pos = window_title.find("#", 1) != std::string::npos)
					window_title.erase(0, pos+1);
				else
					window_title.erase(0, pos+1);
			else
				window_title.erase(0, pos+1);

		ImGui::GetWindowDrawList()->AddText(text_position, color.x > 0 ? ImGui::ColorConvertFloat4ToU32(color) : IM_COL32(0, 0, 0, 255), window_title.c_str());
		if (font)
			PopFont();
		PopClipRect();
	}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImVec2 popup_size)
{
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(2, 0), window->DC.CursorPos + ImVec2(w + 2, label_size.y + style.FramePadding.y*2.0f - 1));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, &id))
		return false;
	...
	if (preview_value != NULL)
		RenderText(ImVec2(frame_bb.Min.x + 6, value_bb.Max.y - 24), preview_value);

	if (label_size.x > 0)
		RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 22), label);
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items) {
	...
	if (items_getter(data, *current_item, &item_text))
		RenderTextClipped(frame_bb.Min + ImVec2(7, 0), value_bb.Max, item_text, NULL, NULL, ImVec2(0.0f, 0.0f));
	}
